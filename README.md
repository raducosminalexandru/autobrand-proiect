...
* **Stocarea Datelor:** Informațiile extrase, precum și datele utilizatorilor, sunt persistate într-o bază de date relațională `PostgreSQL`. Arhitectura stocării este fundamentată pe tabelele principale `produse` (pentru catalogul obținut prin scraping) și `conturi` (pentru managementul accesului și securitate).
